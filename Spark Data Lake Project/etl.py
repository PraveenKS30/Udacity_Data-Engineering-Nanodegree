...
'''
Read config files to extract access key and secret key
'''
...
'''
process_song_data : To process song json files, transform
records, create parquet files and store it on S3 buckets
'''
...
'''
process_log_data : To process log json files, transform
records, create parquet files and store it on S3 buckets
'''
def process_log_data(spark, input_data, output_data):
    ...
    time_table  = df['start_time', 'hour', 'day', 'week', 'month', 'year', 'weekday'].dropDuplicates()
    ...
    df = df.join(song_df, (song_df.title == df.song) & (song_df.duration == df.length))
    songplays_table = df['start_time', 'userId', 'level', 'song_id', 'artist_id', 'sessionId', 'location', 'userAgent'].dropDuplicates()

    # write songplays table to parquet files partitioned by year and month
    songplays_table.write.parquet(os.path.join(output_data, 'songplays_table.parquet'),'overwrite')
